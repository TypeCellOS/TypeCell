/**
 * This is a fork of https://github.com/yjs/y-monaco.
 * 
 * This version adds the ability to change colors and show a text on hover.
 */
...
export class MonacoBinding {
   ...
   */
  constructor(ytext, monacoModel, editors = new Set(), awareness = null, selectionText) {
            ...
            if (clientID !== this.doc.clientID && state.selection != null && state.selection.anchor != null && state.selection.head != null) {
              ...
              if (anchorAbs !== null && headAbs !== null && anchorAbs.type === ytext && headAbs.type === ytext) {
                ...
                if (anchorAbs.index < headAbs.index) {
                  ...
                  afterContentClassName = `yRemoteSelectionHead color-${clientID}`
                  beforeContentClassName = null
                } else {
                  start = monacoModel.getPositionAt(headAbs.index)
                  end = monacoModel.getPositionAt(anchorAbs.index)
                  afterContentClassName = null
                  beforeContentClassName = `yRemoteSelectionHead color-${clientID}`
                }
                newDecorations.push({
                  range: new monaco.Range(start.lineNumber, start.column, end.lineNumber, end.column),
                  options: {
                    className: `yRemoteSelection color-${clientID}`,
                    hoverMessage: { value: state.selection.text },
