...
function generateUserAwarenessCSS(
  clientId: number,
  user: User,
  headSelectionClassName: string,
  selectionClassName: string
) {
  const selectionClassNameForClient = selectionClassName + `-${clientId}`;
  const headSelectionClassNameForClient =
    headSelectionClassName + `-${clientId}`;

  const css =
    `.${selectionClassNameForClient}, .${selectionClassNameForClient}::after, .${selectionClassNameForClient}::before,
       .${headSelectionClassNameForClient}, .${headSelectionClassNameForClient}::after, .${headSelectionClassNameForClient}::before
       {
         background-color: ${user.color} !important;
         border-color: ${user.color} !important;
       }
       
       .${headSelectionClassNameForClient}::after {
         content: '${user.name}'
       }
       `.trim();
      ...
      changedClients.forEach((clientId: number) => {
        ...
        if (state?.user) {
          if (!this.userStore.has(clientId)) {
            const element = generateUserAwarenessCSS(
              clientId,
              state.user,
