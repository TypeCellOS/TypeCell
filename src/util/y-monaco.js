...
export class MonacoBinding {
  ...
  constructor (ytext, monacoModel, editors = new Set(), awareness = null) {
    ...
    this._registerEditor = (editor) => {
        if (this.awareness) {
            this.editors.set(editor, editor.onDidChangeCursorSelection(() => {
            if (editor.getModel() === monacoModel) {
                const sel = editor.getSelection()
                if (sel === null) {
                return
    ...
    editors.forEach(editor => this._registerEditor(editor));
  }
  ...
  removeEditor(editor) {
      let disposer = this.editors.get(editor);
      if (disposer) {
